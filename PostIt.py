...
class SampleApp(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (StartPage, Create, Folders, Settings_page):
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(
            self, text="POST IT!", bg="black", fg="white", font=controller.title_font
        )
        label.grid(column=0, row=0, columnspan=2, sticky=tk.EW, ipady=15)

        img_newPI_folder = Image.open(rf"Images\button_newPI.png")
        img_newPI_resize = img_newPI_folder.resize(
            (80, 80),
        )
        img_newPI = ImageTk.PhotoImage(img_newPI_resize)

        img_folders_folder = Image.open(rf"Images\img_folders.png")
        img_folders_resize = img_folders_folder.resize(
            (60, 60),
        )
        img_folders = ImageTk.PhotoImage(img_folders_resize)

        img_settings_folder = Image.open(rf"Images\img_settings.jpg")
        img_settings_resize = img_settings_folder.resize(
            (80, 80),
        )
        img_settings = ImageTk.PhotoImage(img_settings_resize)

        button_newPI = tk.Button(
            self, image=img_newPI, command=lambda: controller.show_frame("Create")
        )
        button_newPI.grid(column=0, row=1, columnspan=2, sticky=tk.EW)

        button_newPI.image = img_newPI

        button_folders = tk.Button(
            self, image=img_folders, command=lambda: controller.show_frame("Folders")
        )
    ...
    def __init__(self, parent, controller):
        ...
        def NewPI():
            PostIt(
                ...
                txt_style=[txt_txt.get("1.0", "end-1c"), Font.get(), size.get()],
                width=width.get(),
                height=height.get(),
            )
        ...
        create = tk.Button(
            self, text="Create", font=("Arial", 14), borderwidth=0, command=NewPI
        )
...
class PostIt:
    def __init__(self, color, txt_style, width=250, height=250, *args, **kwargs):
        ...
        def round_rectangle(
            x2, y2, Xcolor=color, radius=40, x1=0, y1=0, **kwargs
...
class Settings_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        SizeLabel = tk.Label(
            self, text=controller.traduce("PostIt Size:"), font=SettingsFont
        )
        SizeLabel.grid(row=1, column=0, columnspan=3, sticky="w")

        heightLabel = tk.Label(
            self, text=controller.traduce("Altura:"), font=SettingsFont
        )
        ...
        widthLabel = tk.Label(
            self, text=controller.traduce("Ancho:"), font=SettingsFont
        )
        ...
        FontLabel = tk.Label(
            self, text=controller.traduce("PostIt Font:"), font=SettingsFont
        )
        FontLabel.grid(row=3, column=0, columnspan=3, sticky="w")

        FontLabel = tk.Label(
            self, text=controller.traduce("Fuente:"), font=SettingsFont
        )
        ...
        FontLabel = tk.Label(
            self, text=controller.traduce("Tamaño:"), font=SettingsFont
        )
